...
% Second Layer
conv_out_2 = convol2(conv_out_relu, w2, 1, 2);
for i = 1:size(conv_out_2, 3)
    conv_out_2(:,:,i) = conv_out_2(:,:,i) + b2(i);
end

% Activation
conv_out_2_relu = conv_out_2;
conv_out_2_relu(conv_out_2_relu < 0) = 0;
...
% Thrid Layer
conv_out_3 = convol2(conv_out_2_relu, w3, 1, 2);
for i = 1:size(conv_out_3, 3)
    conv_out_3(:,:,i) = conv_out_3(:,:,i) + b3(i);
end

% Activation
conv_out_3_linear = conv_out_3;

for i = 1:size(conv_out_3, 3)
    figure(1)
    subplot(1,1,1)
    imshow(conv_out_3_linear(:,:,i)); title(['ch = ',num2str(i)]);
