...
% Second Layer
conv_out_2 = convol2(conv_out_relu, wq2, 1, 2);
for i = 1:size(conv_out_2, 3)
    conv_out_2(:,:,i) = conv_out_2(:,:,i) + bq2(i);
end

% Activation
conv_out_2_relu = hwu_relu_quantize(conv_out_2, step, nbits, biases_shift);

for i = 1:size(conv_out_2, 3)
    figure(1)
    subplot(1,2,1)
    imshow(conv_out_2(:,:,i)); title(['ch = ',num2str(i)]);
    subplot(1,2,2)
    imshow(conv_out_2_relu(:,:,i)); title('After ReLU2');
    saveas(gcf,['output/2_',num2str(i),'.bmp']);
    pause(1)
end

% Third Layer
conv_out_3 = convol2(conv_out_2_relu, wq3, 1, 2);
for i = 1:size(conv_out_3, 3)
    conv_out_3(:,:,i) = conv_out_3(:,:,i) + bq3(i);
end

% Activation
conv_out_3_linear = hwu_linear_quantize(conv_out_3, step, nbits, biases_shift);

for i = 1:size(conv_out_3, 3)
    figure(1)
    subplot(1,2,1)
    imshow(conv_out_3(:,:,i)); title(['ch = ',num2str(i)]);
    subplot(1,2,2)
    imshow(conv_out_3_linear(:,:,i)); title('After Linear');
